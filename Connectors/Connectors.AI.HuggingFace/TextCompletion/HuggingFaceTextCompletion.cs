...
public sealed class HuggingFaceTextCompletion : ITextCompletion, IDisposable
{
    ...
    private async Task<IReadOnlyList<ITextCompletionStreamingResult>> ExecuteGetCompletionsAsync(string text, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            List<TextCompletionResponse>? completionResponse;
            try {
                completionResponse = JsonSerializer.Deserialize<List<TextCompletionResponse>>(body);
            } catch (Exception ex) {
            ...
            {
                throw new AIException(AIException.ErrorCodes.InvalidResponseContent, body)
                {
